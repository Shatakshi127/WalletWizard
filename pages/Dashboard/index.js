...
const Form = {
  ...
  validateField() {
    ...
    if (
      ...
      throw new Error("Please fill in required data");
    }
  },
...
const AddChart = {
  ...
  getData() {
    ...
    const hasIncomes = transactions.some(transaction => transaction.amount > 0);
    const hasExpenses = transactions.some(transaction => transaction.amount < 0);
  
    if (hasTransactions && (hasIncomes || hasExpenses)) {
      const labels = transactions.map(transaction => transaction.description || "No Description");
      const amounts = transactions.map(transaction => Math.abs(transaction.amount) / 100);
      const backgroundColors = ["#28D39A", "#ff7782", "#7380EC", "#ffff00","#ffa500","#8080","#ffc0c" ];
